...
| DELETE | `/api/jobs/:jobId` | ADMIN, RECRUITER | → **204** — deletes a **DRAFT** role only (2026-09-21): any in-flight queue work for it (JD generation, samples, a running seal) flips to terminal `CANCELLED` first, then the job, JD draft, blueprint, samples and sealed pools cascade. Errors: **409 `JOB_NOT_DRAFT`** (a published role carries applications and the audit trail — close it instead), **404** |
