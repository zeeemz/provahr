# Graph Report - apps  (2026-08-31)

## Corpus Check
- cluster-only mode — file stats not available

## Summary
- 1237 nodes · 2790 edges · 57 communities
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 27 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `392fcf6f`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- TestSessionScreen.tsx
- lib/http.ts
- session.service.ts
- jobs.router.ts
- evaluation.service.ts
- sandbox/index.ts
- scripts
- ApplicationDetail.tsx
- web/src/api/types.ts
- src/App.tsx
- applications.router.ts
- errMessage
- app.ts
- platform.router.ts
- prisma.ts
- blueprint.service.ts
- oidc.ts
- TestFlow.tsx
- expo
- JobConsole.tsx
- item.ts
- compilerOptions
- AppError
- devDependencies
- session-draw.test.ts
- src/types.ts
- draw.ts
- auth.ts
- auth-config.service.ts
- setup.router.ts
- evaluation-routes.test.ts
- session-routes.test.ts
- auth.service.ts
- compilerOptions
- dependencies
- compilerOptions
- ProvidersPage.tsx
- devDependencies
- blueprint-routes.test.ts
- tsconfig.build.json
- SettingsPage.tsx
- JobDetail.tsx
- include
- mobile/package.json
- dependencies
- devDependencies
- web/package.json
- exclude
- worker/package.json
- seed.ts
- scripts
- scripts
- PlatformPage

## God Nodes (most connected - your core abstractions)
1. `AppError` - 110 edges
2. `errMessage()` - 48 edges
3. `prisma` - 24 edges
4. `humanize()` - 23 edges
5. `useAuth()` - 18 edges
6. `createApp()` - 16 edges
7. `newItemId()` - 16 edges
8. `fmtDateTime()` - 15 edges
9. `compilerOptions` - 15 edges
10. `Spinner()` - 14 edges

## Surprising Connections (you probably didn't know these)
- `ipRateLimit()` --calls--> `AppError`  [EXTRACTED]
  api/src/modules/public/public.router.ts → api/src/lib/http.ts
- `CodeReviewPromptInput` --references--> `CodeLanguage`  [EXTRACTED]
  api/src/prompts/evaluation.ts → api/src/lib/assessment/item.ts
- `changeStatus()` --calls--> `errMessage()`  [EXTRACTED]
  web/src/hr/ApplicationDetail.tsx → web/src/api/client.ts
- `moveStage()` --calls--> `errMessage()`  [EXTRACTED]
  web/src/hr/ApplicationDetail.tsx → web/src/api/client.ts
- `JdStep()` --calls--> `humanize()`  [EXTRACTED]
  web/src/hr/JobConsole.tsx → web/src/components/ui.tsx

## Import Cycles
- None detected.

## Communities (57 total, 0 thin omitted)

### Community 0 - "TestSessionScreen.tsx"
Cohesion: 0.05
Nodes (68): App(), Route, styles, api, ApiError, ApiErrorBody, apiUrl(), DEFAULT_API_URL (+60 more)

### Community 1 - "lib/http.ts"
Cohesion: 0.08
Nodes (44): decryptSecret(), deriveKey(), encryptSecret(), AnthropicAdapter, buildAnthropicMessages(), ContentBlock, parseAnthropicResponse(), AzureOpenAiAdapter (+36 more)

### Community 2 - "session.service.ts"
Cohesion: 0.06
Nodes (61): createRateLimiter(), RateLimiterOptions, deadlineFor(), isExpired(), remainingMs(), SUBMIT_GRACE_MS, withinSubmitGrace(), generateTestToken() (+53 more)

### Community 3 - "jobs.router.ts"
Cohesion: 0.06
Nodes (52): assertPublicHttpUrl(), extractText(), fetchPageText(), isAcceptableContentType(), isPrivateIpv4(), isPrivateIpv6(), parseIpv4(), parseIpv6() (+44 more)

### Community 4 - "evaluation.service.ts"
Cohesion: 0.06
Nodes (53): backoffMs(), claimNext(), complete(), fail(), JobType, QueueJob, requeueStale(), McqAnswerLike (+45 more)

### Community 5 - "sandbox/index.ts"
Cohesion: 0.12
Nodes (32): CODE_LANGUAGES, CodeLanguage, assertHardenedArgs(), buildRunArgs(), CANONICAL_PREFIXES, COMMAND_PREFIX, IMAGE_ALLOW_LIST, stdinPayload() (+24 more)

### Community 6 - "scripts"
Cohesion: 0.05
Nodes (42): dependencies, bcryptjs, cors, express, helmet, jsonwebtoken, morgan, @prisma/client (+34 more)

### Community 7 - "ApplicationDetail.tsx"
Cohesion: 0.10
Nodes (33): api, asPresented(), DashboardStats, IntakeInput, IntakeResponse, IntakeScreenshot, Job, Stage (+25 more)

### Community 8 - "web/src/api/types.ts"
Cohesion: 0.06
Nodes (36): AiLikelihood, ApplicationDetail, ApplicationListItem, ApplicationStatus, AuthMode, AuthResponse, BlueprintDto, Candidate (+28 more)

### Community 9 - "src/App.tsx"
Cohesion: 0.10
Nodes (27): ROLE_BADGE_CLASS, ROLE_EXPLAINERS, ROLES, TeamPage(), ApiError, ApiErrorBody, ApiErrorDetail, getToken() (+19 more)

### Community 10 - "applications.router.ts"
Cohesion: 0.12
Nodes (28): router, changeStatusSchema, listApplicationsQuerySchema, moveStageSchema, StatusAction, changeStatus(), getDetail(), getScopedApplication() (+20 more)

### Community 11 - "errMessage"
Cohesion: 0.09
Nodes (32): AddProviderForm(), submit(), ProvidersPage(), activate(), confirmDelete(), test(), InviteForm(), submit() (+24 more)

### Community 12 - "app.ts"
Cohesion: 0.09
Nodes (21): createApp(), boolString, DEV_DEFAULT_SECRETS_KEY, env, parsed, schema, usesUnsafeProductionSecrets(), app (+13 more)

### Community 13 - "platform.router.ts"
Cohesion: 0.11
Nodes (25): slugify(), createCompany(), deleteCompany(), freeSlug(), listCompanies(), patchCompany(), PlatformCompanyRow, requireSuperAdmin() (+17 more)

### Community 14 - "prisma.ts"
Cohesion: 0.09
Nodes (20): SignExpiresIn, signToken(), prisma, adminA, adminB, app, authAs(), companyA (+12 more)

### Community 15 - "blueprint.service.ts"
Cohesion: 0.17
Nodes (26): countByFormat(), drawSizes(), poolSatisfiesBlueprint(), requiredPoolSizes(), getActiveAdapter(), enqueue(), enqueueEvaluation(), activePoolFor() (+18 more)

### Community 16 - "oidc.ts"
Cohesion: 0.12
Nodes (14): isJwksDocument(), Jwk, JwksCache, jwksCaches, JwksDocument, nonEmptyString(), OidcTokenInfo, stringArray() (+6 more)

### Community 17 - "TestFlow.tsx"
Cohesion: 0.10
Nodes (19): AnswerContent, SessionView, SignalType, SwipeValuation, TestLinkInfo, mmss(), asMcqSaved(), asSwipeSaved() (+11 more)

### Community 18 - "expo"
Cohesion: 0.09
Nodes (22): softwareKeyboardLayoutMode, usesCleartextTraffic, expo, android, assetBundlePatterns, backgroundColor, extra, ios (+14 more)

### Community 19 - "JobConsole.tsx"
Cohesion: 0.13
Nodes (19): BlueprintSection, BlueprintStatusView, JdDraft, JdView, PoolStatusView, QuestionFormat, SampleItem, DIFFICULTY_MIXES (+11 more)

### Community 20 - "item.ts"
Cohesion: 0.10
Nodes (20): codeItem, difficultySchema, formatCount, formatsShape, hiddenCaseSchema, itemId, mcqItem, mcqOptionSchema (+12 more)

### Community 21 - "compilerOptions"
Cohesion: 0.09
Nodes (21): DOM, DOM.Iterable, vite.config.ts, compilerOptions, isolatedModules, jsx, lib, module (+13 more)

### Community 22 - "AppError"
Cohesion: 0.19
Nodes (16): AppError, router, CreateInterviewInput, createInterviewSchema, interviewTypeSchema, listInterviewsQuerySchema, ScorecardInput, scorecardSchema (+8 more)

### Community 23 - "devDependencies"
Cohesion: 0.11
Nodes (19): devDependencies, supertest, tsx, @types/cors, @types/express, @types/jsonwebtoken, @types/morgan, @types/node (+11 more)

### Community 24 - "session-draw.test.ts"
Cohesion: 0.19
Nodes (15): newItemId(), QuestionFormat, ALL_FORMATS, codeItem(), mcqItem(), swipeItem(), writtenItem(), codeItem() (+7 more)

### Community 25 - "src/types.ts"
Cohesion: 0.22
Nodes (10): asyncHandler(), Request, router, dashboard(), CreateUserInput, createUserSchema, createCompanyUser(), listCompanyUsers() (+2 more)

### Community 26 - "draw.ts"
Cohesion: 0.21
Nodes (14): AssessmentItem, BlueprintSection, DrawInput, DrawnQuestion, drawSession(), hashSeed(), KeyAbsent, mulberry32() (+6 more)

### Community 27 - "auth.ts"
Cohesion: 0.26
Nodes (14): getJwksCache(), verifyToken(), Express, extractUnverifiedIssuer(), LoadedUser, loadUserById(), localAuth(), oidcAuth() (+6 more)

### Community 28 - "auth-config.service.ts"
Cohesion: 0.19
Nodes (12): requireRole(), router, PutAuthConfigInput, putAuthConfigSchema, authConfigSelect, AuthConfigView, getCompanyAuthConfig(), hasAnyEnabledAuthConfig() (+4 more)

### Community 29 - "setup.router.ts"
Cohesion: 0.20
Nodes (11): register(), hits, NOTE: helmet's default CSP blocks inline <script>, so the page's JS is a, router, WIZARD_HTML, WIZARD_JS, InstallInput, installSchema (+3 more)

### Community 30 - "evaluation-routes.test.ts"
Cohesion: 0.13
Nodes (10): app, auth, ENCRYPTED_POOL, {
  mockUser,
  savedEvaluations,
  userFindUnique,
  applicationFindUnique,
  applicationUpdate,
  testSessionFindUnique,
  sessionQuestionFindFirst,
  sessionQuestionFindMany,
  answerUpsertUnused,
  poolFindFirst,
  voidedItemFindMany,
  voidedItemUpsert,
  voidedItemFindUnique,
  llmProviderFindFirst,
  evaluationFindUnique,
  evaluationUpsert,
  evaluationFindMany,
  evaluationUpdateMany,
  executionResultUpsert,
  sessionAssessmentUpsert,
  sessionAssessmentFindUnique,
  signalFindMany,
}, now, POOL_ITEMS, NOTE: the collusion probe also carries sessionId ({ not: ... }) — branch, SESSION_QUESTIONS (+2 more)

### Community 31 - "session-routes.test.ts"
Cohesion: 0.15
Nodes (10): assessmentItemSchema, app, BLUEPRINT, ENCRYPTED_POOL, POOL_IDS, POOL_ITEMS, PRESENTED_MCQ, sessionRow() (+2 more)

### Community 32 - "auth.service.ts"
Cohesion: 0.26
Nodes (9): hashPassword(), verifyPassword(), router, LoginInput, loginSchema, RegisterInput, registerSchema, DUMMY_HASH (+1 more)

### Community 33 - "compilerOptions"
Cohesion: 0.14
Nodes (14): compilerOptions, esModuleInterop, forceConsistentCasingInFileNames, lib, module, noImplicitOverride, outDir, resolveJsonModule (+6 more)

### Community 34 - "dependencies"
Cohesion: 0.15
Nodes (13): expo, expo-clipboard, expo-constants, expo-status-bar, dependencies, expo, expo-clipboard, expo-constants (+5 more)

### Community 35 - "compilerOptions"
Cohesion: 0.15
Nodes (12): compilerOptions, noFallthroughCasesInSwitch, noUnusedLocals, noUnusedParameters, strict, types, extends, include (+4 more)

### Community 36 - "ProvidersPage.tsx"
Cohesion: 0.15
Nodes (11): BASE_URL_PLACEHOLDER, KIND_HELPER, KIND_LABEL, KINDS, ProviderRow(), TestOutcome, TEXT_MODEL_PLACEHOLDER, CreateProviderInput (+3 more)

### Community 37 - "devDependencies"
Cohesion: 0.20
Nodes (10): typescript, typescript, @babel/core, devDependencies, @babel/core, @types/react, typescript, @types/react (+2 more)

### Community 38 - "blueprint-routes.test.ts"
Cohesion: 0.22
Nodes (8): blueprintSectionSchema, PutBlueprintInput, putBlueprintSchema, samplesRequestSchema, app, auth, token, validBlueprint

### Community 39 - "tsconfig.build.json"
Cohesion: 0.25
Nodes (7): compilerOptions, outDir, rootDir, extends, include, src/**/*.ts, ./tsconfig.json

### Community 40 - "SettingsPage.tsx"
Cohesion: 0.29
Nodes (7): issuerShapeValid(), KeycloakConfigCard(), submit(), SettingsPage(), AuthModeResponse, CompanyAuthConfig, ApiErrorScreen()

### Community 41 - "JobDetail.tsx"
Cohesion: 0.29
Nodes (7): isNotFound(), ApplyInput, ApplyResponse, PublicJob, EMPTY_FORM, FormState, JobDetail()

### Community 42 - "include"
Cohesion: 0.29
Nodes (6): exclude, include, dist, src/**/*.ts, prisma/**/*.ts, tests/**/*.ts

### Community 43 - "mobile/package.json"
Cohesion: 0.29
Nodes (6): description, license, main, name, private, version

### Community 44 - "dependencies"
Cohesion: 0.29
Nodes (7): react-dom, react-router-dom, dependencies, react, react-dom, react-router-dom, react

### Community 45 - "devDependencies"
Cohesion: 0.29
Nodes (7): @types/react-dom, vite, @vitejs/plugin-react, devDependencies, @types/react-dom, vite, @vitejs/plugin-react

### Community 46 - "web/package.json"
Cohesion: 0.29
Nodes (6): description, license, name, private, type, version

### Community 47 - "exclude"
Cohesion: 0.33
Nodes (6): prisma, exclude, dist, node_modules, prisma, tests

### Community 48 - "worker/package.json"
Cohesion: 0.33
Nodes (5): description, license, name, private, version

### Community 49 - "seed.ts"
Cohesion: 0.60
Nodes (4): daysAgo(), daysFromNow(), main(), prisma

### Community 50 - "scripts"
Cohesion: 0.40
Nodes (5): scripts, android, ios, start, typecheck

### Community 51 - "scripts"
Cohesion: 0.40
Nodes (5): scripts, build, dev, preview, typecheck

### Community 52 - "PlatformPage"
Cohesion: 1.00
Nodes (3): PlatformPage(), deleteCompany(), reload()

## Knowledge Gaps
- **359 isolated node(s):** `Route`, `ApiErrorBody`, `EmploymentType`, `QuestionFormat`, `RoleFamily` (+354 more)
  These have ≤1 connection - possible missing edges or undocumented components.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `AppError` connect `AppError` to `auth.service.ts`, `lib/http.ts`, `session.service.ts`, `jobs.router.ts`, `evaluation.service.ts`, `sandbox/index.ts`, `applications.router.ts`, `app.ts`, `platform.router.ts`, `prisma.ts`, `blueprint.service.ts`, `oidc.ts`, `src/types.ts`, `auth.ts`, `auth-config.service.ts`, `setup.router.ts`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `prisma` connect `prisma.ts` to `auth.service.ts`, `lib/http.ts`, `session.service.ts`, `jobs.router.ts`, `evaluation.service.ts`, `applications.router.ts`, `app.ts`, `platform.router.ts`, `blueprint.service.ts`, `AppError`, `src/types.ts`, `auth.ts`, `auth-config.service.ts`, `setup.router.ts`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Why does `devDependencies` connect `devDependencies` to `devDependencies`, `scripts`, `exclude`?**
  _High betweenness centrality (0.011) - this node is a cross-community bridge._
- **What connects `Route`, `ApiErrorBody`, `EmploymentType` to the rest of the system?**
  _359 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `TestSessionScreen.tsx` be split into smaller, more focused modules?**
  _Cohesion score 0.051590483827853514 - nodes in this community are weakly interconnected._
- **Should `lib/http.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.08490945674044266 - nodes in this community are weakly interconnected._
- **Should `session.service.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.056265984654731455 - nodes in this community are weakly interconnected._